...
def make_fixtures():
    """Two families with one slot each. Family A has both parents on file."""
    ...
    session.add(team)
    session.flush()
    a = _family(session, team, "Klara", "a@exempel.se", "a2@exempel.se")
...
def test_propose_swap_creates_a_pending_request():
    ...
    request = propose_swap(
        ...
        proposer_person_id=a.parents[0].id, now=datetime(2026, 1, 1),
    )

    assert request.status == "pending"


def test_either_parent_may_propose_on_the_familys_behalf():
    session, a, b, slot_a, slot_b = make_fixtures()

    request = propose_swap(
        session, proposer_slot_id=slot_a.id, target_slot_id=slot_b.id,
        proposer_person_id=a.parents[1].id, now=datetime(2026, 1, 1),
    )
...
def test_propose_swap_rejects_a_target_slot_the_family_already_owns():
    """Including one the *other* parent would have been shown as theirs."""
    session, a, b, slot_a, slot_b = make_fixtures()
    from datetime import date, time
...
def test_accept_swap_exchanges_the_two_slots_player_id():
    session, a, b, slot_a, slot_b = make_fixtures()
    request = propose_swap(
        session, proposer_slot_id=slot_a.id, target_slot_id=slot_b.id,
        proposer_person_id=a.parents[0].id, now=datetime(2026, 1, 1),
    ...
    accept_swap(
        session, request.id, accepting_person_id=b.parents[0].id,
        now=datetime(2026, 1, 2),
    )

    session.refresh(slot_a)
    session.refresh(slot_b)
    assert slot_a.player_id == b.id
    assert slot_b.player_id == a.id
    assert session.get(SwapRequest, request.id).status == "accepted"


def test_accept_swap_rejects_someone_from_another_family():
    session, a, b, slot_a, slot_b = make_fixtures()
    request = propose_swap(
        session, proposer_slot_id=slot_a.id, target_slot_id=slot_b.id,
        proposer_person_id=a.parents[0].id, now=datetime(2026, 1, 1),
    ...
    with pytest.raises(SwapError, match="inte ditt"):
        accept_swap(
            session, request.id, accepting_person_id=a.parents[0].id,
            now=datetime(2026, 1, 2),
        )


def test_the_second_parent_to_answer_gets_a_clean_error_not_a_double_swap():
    """Both parents see the same pending request, so both can click it."""
    session, a, b, slot_a, slot_b = make_fixtures()
    b.parents.append(Person(name="B2", email="b2@exempel.se"))
    session.commit()
    request = propose_swap(
        session, proposer_slot_id=slot_a.id, target_slot_id=slot_b.id,
        proposer_person_id=a.parents[0].id, now=datetime(2026, 1, 1),
    ...
    with pytest.raises(SwapError, match="redan avgjort"):
        accept_swap(
            session, request.id, accepting_person_id=b.parents[1].id,
            now=datetime(2026, 1, 2),
        )
...
def test_decline_swap_marks_it_declined_without_moving_anyone():
    ...
    decline_swap(
        session, request.id, declining_person_id=b.parents[0].id,
        now=datetime(2026, 1, 2),
    )

    session.refresh(slot_a)
    assert slot_a.player_id == a.id
...
def test_accepting_a_second_offer_on_an_already_swapped_slot_is_rejected():
    ...
    accept_swap(
        session, request_to_b.id, accepting_person_id=b.parents[0].id,
        now=datetime(2026, 1, 2),
    )
    session.refresh(slot_a)
    session.refresh(slot_b)
    ...
    with pytest.raises(SwapError):
        accept_swap(
            session, request_to_c.id, accepting_person_id=c.parents[0].id,
            now=datetime(2026, 1, 3),
        )

    session.refresh(slot_a)
    session.refresh(slot_c)
    assert slot_a.player_id == b.id, "B's legitimately-received slot must not move"
    assert slot_c.player_id == c.id
