...
def test_post_swaps_creates_a_pending_request_and_notifies_the_target(
    client, app, session_factory, seeded, monkeypatch
, csrf_token):
    notified = []
    monkeypatch.setattr(
...
, csrf_token):
    ...
    monkeypatch.setattr(
        ...
        lambda to, **kwargs: notified.append((to, kwargs)),
    )
    ...
    assert proposer_slot.person_id == target_person.id
    assert len(notified) == 1
    to, kwargs = notified[0]
    assert to == seeded["person"].email
    # The accepter is the target person (Sara) — a bug in the original
    ...
    assert kwargs["accepter_name"] == target_person.name
    ...
    # the rows after the person_id swap already happened) that a mere
...
, csrf_token):
    ...
    assert response.status_code == 302
    proposer_slot = session.get(Slot, seeded["slot"].id)
    assert proposer_slot.person_id == seeded["person"].id
    assert len(notified) == 1
    to, kwargs = notified[0]
    assert to == seeded["person"].email
    # The decliner is the target person (Sara), not the proposer — a bug
    # in the original draft passed the proposer's own name here.
    assert kwargs["decliner_name"] == target_person.name
...
def test_post_swaps_with_an_unfilled_target_slot_does_not_crash_or_create_a_request(
    client, app, session_factory, seeded
, csrf_token):
    ...
    crash reading target_slot.person.email (I3)."""
    session = session_factory()
    from datetime import date, time
