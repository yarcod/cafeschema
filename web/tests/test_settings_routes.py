...
def test_opted_out_parent_gets_no_swap_proposal_mail(
    ...
    bea.parents[0].email_notifications = False
    theirs = Slot(
...
def test_opted_in_parent_does_get_the_swap_proposal_mail(
    ...
    monkeypatch.setattr(
        "duty_web.routes.swap_routes.send_swap_proposed",
        lambda emails, **kwargs: sent.extend(emails),
    )
    _login(client, seeded["person"])
...
def test_swap_proposal_mail_links_to_the_page_where_you_answer(
    ...
    monkeypatch.setattr(
        "duty_web.routes.swap_routes.send_swap_proposed",
        lambda emails, **kwargs: sent.update(kwargs),
    )
    _login(client, seeded["person"])
...
def test_only_the_opted_in_parent_of_a_household_is_mailed(
    client, seeded, session_factory, monkeypatch, csrf_token
):
    """The opt-out is per parent, not per family — one of them saying no
    must not silence the other, or mute the household entirely."""
    session = session_factory()
    bea = make_family(
        session, seeded["team"], "Bea Bengtsson",
        [("Bo Bengtsson", "bo@exempel.se"), ("Bea Senior", "senior@exempel.se")],
    )
    bea.parents[0].email_notifications = False
    theirs = Slot(
        team_id=seeded["team"].id, date=date(2026, 12, 5), start_time=time(9, 0),
